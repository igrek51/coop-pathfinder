...
public class SimpleAstarController {
	...
	private void findPathPressed(final Event event) {
		...
		PointCoordinates target = findFirstCellTypeButNot(map, TileCellType.START, start);
	...
	private boolean contains(PointCoordinates seek, PointCoordinates... collection) {
		for (PointCoordinates c : collection) {
			if (c.equals(seek))
				return true;
		}
		return false;
	...
	private PointCoordinates findFirstCellTypeButNot(TestTileMap map, TileCellType wantedType, PointCoordinates... excludePoints) {
		for (int x = 0; x < map.getWidthInTiles(); x++) {
			for (int y = 0; y <= map.getHeightInTiles(); y++) {
				TileCellType type = map.get(x, y);
				if (type == wantedType) {
					PointCoordinates p = new PointCoordinates(x, y);
